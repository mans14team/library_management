...
#### 1. 주요 성능 지표
| 지표 | 캐싱 적용 전 | 캐싱 + 인덱스 적용 후 | 개선점 및 차이 |
|------|------------|-------------------|-------------|
| **평균 응답 시간** | 3239ms | 1969ms | 약 **39% 감소** |
| **중간값 (Median)** | 3718ms | 2163ms | 약 **42% 감소** |
| **90% 응답 시간** | 5665ms | 2943ms | 약 **48% 감소** |
| **95% 응답 시간** | 6117ms | 3069ms | 약 **50% 감소** |
| **99% 응답 시간** | 6725ms | 3270ms | 약 **51% 감소** |
| **최소 응답 시간** | 8ms | 7ms | 큰 차이 없음 |
| **최대 응답 시간** | 7226ms | 3527ms | 약 **51% 감소** |
| **처리량** | 478.4 req/sec | 886.2 req/sec | 약 **85% 증가** |
| **수신 KB/초** | 1368.52 KB/sec | 3526.77 KB/sec | 약 **158% 증가** |
| **전송 KB/초** | 188.29 KB/sec | 349.97 KB/sec | 약 **86% 증가** |

### B. 게시글 상세 조회 성능 비교

#### 1. 주요 성능 지표
| 지표 | 캐싱 적용 전 | 캐싱 적용 후 | 개선점 및 차이 |
|------|------------|-------------|-------------|
| **평균 응답 시간** | 3942ms | 1256ms | 약 **68% 감소** |
| **중간값 (Median)** | 4095ms | 1257ms | 약 **69% 감소** |
| **90% 응답 시간** | 5098ms | 2154ms | 약 **58% 감소** |
| **95% 응답 시간** | 5454ms | 2290ms | 약 **58% 감소** |
| **99% 응답 시간** | 6548ms | 2609ms | 약 **60% 감소** |
| **최소 응답 시간** | 22ms | 7ms | 약 **68% 감소** |
| **최대 응답 시간** | 7018ms | 3140ms | 약 **55% 감소** |
| **처리량** | 304.7 req/sec | 503.5 req/sec | 약 **65% 증가** |
| **수신 KB/초** | 465.33 KB/sec | 769.02 KB/sec | 약 **65% 증가** |
| **전송 KB/초** | 115.44 KB/sec | 190.78 KB/sec | 약 **65% 증가** |

### 2. 종합 개선 효과

<details>
<summary><b>응답 속도 개선</b></summary>

- **목록 조회**: 평균 응답 시간 39% 감소
- **상세 조회**: 평균 응답 시간 68% 감소
- 전반적인 사용자 체감 속도 대폭 개선
- 응답 시간의 일관성 확보
</details>

<details>
<summary><b>시스템 처리량 향상</b></summary>

- **목록 조회**: 처리량 85% 증가 (478.4 → 886.2 req/sec)
- **상세 조회**: 처리량 65% 증가 (304.7 → 503.5 req/sec)
- 더 많은 동시 사용자 처리 가능
</details>

<details>
<summary><b>네트워크 효율성 개선</b></summary>

- **목록 조회**: 데이터 전송량 최대 158% 증가
- **상세 조회**: 데이터 처리량 65% 증가
- 전반적인 시스템 처리 효율성 향상
</details>

<details>
<summary><b>시스템 안정성 강화</b></summary>

- **목록 조회**: 최대 응답 시간 51% 감소
...
| 정원석 | 부팀장 | **▶ 도서 정보 관리**<br>- 도서 CRUD 기능 구현<br>- 도서 정보의 물리적, 메타적 데이터 분리<br><br>**▶ 도서 예약**<br>- 도서 예약 상태 관리<br>- 낙관적 락 및 특정 시간대 상태 변경 차단으로 인한 동시성 제어<br><br>**▶ 도서 검색**<br>- QueryDsl 기반 세부 필드값 입력 검색 기능 구현<br><br>**▶ 태그 검색(예정)**<br>- 캐싱을 통한 빠른 태그 조회 | [Github](https://github.com/Aakaive) |
| 이동휘 | 팀원 | **▶ 유저**<br>- 회원가입/로그인<br>- 유저 CRUD<br>- Spring Security 필터<br>- @PreAuthorize를 활용한 권한관리<br>- Redis를 활용한 Refresh Token 관리<br>- OAUTH2.0을 이용한 카카오, 네이버 로그인<br><br>**▶ 게시글**<br>- 게시글 CRUD<br>- 게시글 댓글 CRUD<br>- 권한에 따른 게시글 작성 권한 구분<br><br>**▶ 멤버십**<br>- 토스 페이를 활용한 멤버십 구현<br>- 멤버십 및 결제 내역 CRUD 구현<br><br>**▶ CI/CD**<br>- AWS Elastic Beanstalk 활용<br>- RDS를 통해 MySQL 사용<br>- ECR 활용해 이미지 관리<br>- ElastiCache 활용해 Redis 관리<br>- Github Actions를 활용한 배포 <br>- 로그 관리를 위한 ELK 스택 구현 완료 <br><br>**▶ 알림**<br> - 알림 비동기 전환을 위한 RabbitMQ 구현<br><br>**▶ 도서 검색 고도화**<br>- ElasticSearch를 이용한 검색 기능 구현<br>- 자동완성 기능 구현<br>- 연관 검색어 기능 구현 | [Github](https://github.com/webstrdy00) |
